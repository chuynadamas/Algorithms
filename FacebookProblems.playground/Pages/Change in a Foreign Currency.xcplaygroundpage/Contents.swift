...
func canGetExactChange(targetMoney: Int, denominations: [Int]) -> Bool {
    ...
    func backtrack(_ newTarget: Int) -> Bool {
        if targetMoney == 0 {
            return true
        }
        ...
        for coin in denominations {
            result = result || backtrack(newTarget - coin)
        }
        
        return result
    }
    
    return backtrack(targetMoney);
}


canGetExactChange(targetMoney:94, denominations:[5,10,25,100,200])
//canGetExactChange(targetMoney:75, denominations:[4,17,29])
